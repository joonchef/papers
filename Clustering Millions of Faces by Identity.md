...
더욱이 우리는 짧은 k-list의리스트 만 고려한다면 짧은리스트의 특정 예제의 존재 여부가 샘플의 수치 적 순위보다 더 중요 할 수 있음을 알 수있다. 이와 같이, 랭크보다는 공유 된 가장 가까운 이웃의 존재 / 부존재를 직접 합산하는 것에 기초한 거리 측정을 고려하여 다음과 같은 거리 함수
